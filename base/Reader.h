...
void importNegativeSamples() {
	FILE *fin;
	int tmp;
	fin = fopen((inPath + "diff2id.txt").c_str(), "r");

	if (fin == nullptr) {
		std::cout << '`' << inPath << "diff2id.txt" << '`' << " does not exist"
		          << std::endl;
		return;
	}

	tmp = fscanf(fin, "%ld", &diffTotal);
	printf("The total of negative samples is %ld.\n", diffTotal);


	trueNegList = (Triple *)calloc(diffTotal, sizeof(Triple));
